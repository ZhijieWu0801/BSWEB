<template>
    <div class="petInboundContainer">
        <div>
            <span>验证管理员</span>
            <input type="text" v-model="verifyTel">
            <button @click.stop="verifyHandle">点击验证</button>
        </div>
      <Dialogs
        @Ok="okHandle"
        @Cancel="CancelHandle"
  ...
  export default {
    ...
    verifyHandle(){
          this.$axios.post(`${this.$baseUrl}/api/admin/getAdminByTel`, {tel:this.verifyTel}).then((r)=>{
            console.log(r);
          })
